...
VECT_ARROW_TO_DB2_TIME & MY_DICT::getTestTime()
{
    return *((VECT_ARROW_TO_DB2_TIME *)m_vector);
}
...
MY_DICT::~MY_DICT()
{
    ...
    if (m_vector != NULL)
    {
        switch (this->vector_type)
        {
            ...
            default:
            {
                printf("%d %s() error dont know what type %d\n", 
                    __LINE__, 
                    __FUNCTION__,
                    this->vector_type);
                break;
...
ostream& operator<<(ostream& os, MY_DICT& str_d)
{
    unsigned long i = 0;
    os << str_d.to_string(i).c_str();
    return os;
}
...
string MY_DICT::to_string(size_t i)
{
    ...
    switch (vector_type)
    {
        ...
        case arrow::Type::UINT32:
            ...
            break;
        case arrow::Type::DATE32:
            stringStreamlog << getTestTime()[i].to_string_as_date();
            break;
        case arrow::Type::TIME32:
            stringStreamlog << getTestTime()[i].to_string_as_time();
            break;
        case arrow::Type::TIMESTAMP:
        case arrow::Type::DATE64:
        case arrow::Type::TIME64:
            stringStreamlog << getTestTime()[i];
            break;
        case arrow::Type::INT16:
...
void free_MAP_FIELDNAME_DICT(MAP_FIELDNAME_DICT &map_field_memory_vectors)
{
    // free STRUCT_DICT
